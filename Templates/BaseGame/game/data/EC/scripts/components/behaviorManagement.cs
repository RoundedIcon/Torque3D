...
{
	/*if(%componentName !$= "")
		echo("Creating a new component: " @ %componentName);
	else
		echo("Creating a new component: " @ %componentClass);
		
	//create a dummy instance we can copy from, as well as registering it into the editor
	%command = "new" SPC %componentClass SPC "(" @ %componentName @ "){ enabled = false; friendlyName = \"" 
	@ %friendlyName @ "\"; componentType = \"" @ %componentType @ "\"; networked = \"" @ %networked 
...
function BehaviorManager::checkForDependants(%behavior)
{
   ...
   %conflictNum = 0;
   %conflictList = "";
   for(%i = 0; %i < %behavior.owner.getComponentCount(); %i++)
   {
      %chkBhvrI = %behavior.owner.getComponentByIndex(%i);
...
function Component::onBehaviorAdd( %this )
{
   ...
   for(%i = 0; %i < %this.owner.getComponentCount(); %i++)
   {
      %behavior = %this.owner.getComponentByIndex(%i);
...
function BehaviorComponent::onBehaviorsLoaded( %this )
{
   ...
   {
      %behavior = %this.getComponentByIndex(%i);
      %conflictList = BehaviorManager::checkForUnmetDependencies( %behavior );
