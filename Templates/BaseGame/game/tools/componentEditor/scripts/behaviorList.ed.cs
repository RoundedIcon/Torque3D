function GuiInspectorEntityGroup::createAddBehaviorList(%this)
{
...
function QuickEditBehaviorList::refresh(%this)
{
   ...
   // Find all the types.
   %count = $ComponentEditor::componentTypesCount;

   //%count = BehaviorSet.getCount();
   %categories = "";
   for (%i = 0; %i < %count; %i++)
   {
	  %item = $ComponentEditor::componentTypes[%i];
      //%item = BehaviorSet.getObject(%i);
	  %compType = getField(%item, 3);
      if (!isInList(%compType, %categories))
         %categories = %categories TAB %compType;
   }
   ...
   for (%i = 0; %i < %categoryCount; %i++)
   {
      ...
      for (%j = 0; %j < %count; %j++)
      {
         %item = $ComponentEditor::componentTypes[%j];
      	 //%item = BehaviorSet.getObject(%j);
	  	 %compType = getField(%item, 3);
		 %friendName = getField(%item, 2);
...
function QuickEditBehaviorList::onHotTrackItem( %this, %itemID )
{
   ...
   if( isObject( %behaviorObj ) && %this.behaviorDesc != %behaviorObj )
   {
      ...
      %fieldCount = %behaviorObj.getComponentFieldCount();
      for (%i = 0; %i < %fieldCount; %i++)
      {
         %name = getField(%behaviorObj.getComponentField(%i), 0);
...
function AddBehaviorQuickEditButton::onClick(%this)
{
   ...
   //   return;
	...
	%componentName = getField(%this.behaviorList.behaviorToAdd, 0);
	%componentClass = getField(%this.behaviorList.behaviorToAdd, 1);
