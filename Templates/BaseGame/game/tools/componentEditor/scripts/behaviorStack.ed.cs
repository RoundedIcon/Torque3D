function BehaviorEditor::createBehaviorRollout(%this, %component)
{
   ...
   %checkBox.setStateOn(%component.enabled);
   
   if(getWordCount(%component.missingDependencies) > 0)
   {
      for(%i=0; %i < getWordCount(%component.missingDependencies); %i++)
      {
         %missing = %missing NL "Behavior missing:" SPC getWord(%component.missingDependencies, %i);
      }
   ...
   %count = %component.getComponentFieldCount();
   for( %i = 0; %i < %count; %i++ )
   {
      //we skip the enabled field because we integrate that into the rollout
      %name = getField(%component.getComponentField(%i), 0);
      if(%name $= "enabled")
         continue;
         
      BehaviorEditor::createFieldGui(%fieldContainer, %component, %i);
   }
